...
import type {
  PutCreditSpreadGeneratedCandidate,
  PutCreditSpreadRuleset,
  RawOptionContractRow,
} from "@/lib/options-data/types";

const FIRST_PUT_CREDIT_SPREAD_RULESET: PutCreditSpreadRuleset = {
  minDte: 7,
  maxDte: 30,
  allowedWidths: [1, 2, 5],
  minPercentBelowSpot: 0.02,
  maxPercentBelowSpot: 0.12,
  minNetCredit: 0.1,
...
function percentBelowSpot(underlyingPrice: number, shortStrike: number): number {
  return (underlyingPrice - shortStrike) / underlyingPrice;
}

function scoreDownsideBuffer(pctBelowSpot: number): number {
  const scaled = pctBelowSpot / FIRST_PUT_CREDIT_SPREAD_RULESET.maxPercentBelowSpot;
...
function buildSetupLabel(
  ...
  return `${ticker} ${expiration} ${shortStrike}/${longStrike} Put Credit Spread`;
}

function labelRoles(
  candidates: PutCreditSpreadGeneratedCandidate[]
): PutCreditSpreadGeneratedCandidate[] {
  if (candidates.length === 0) return [];

  const bySafety = [...candidates].sort((a, b) => a.shortStrike - b.shortStrike);
  ...
  const sortedByShortStrike = [...candidates].sort((a, b) => a.shortStrike - b.shortStrike);
...
function preSortCandidates(
  ...
  return [...candidates].sort((a, b) => {
    const aDistance = percentBelowSpot(underlyingPrice, a.shortStrike);
    const bDistance = percentBelowSpot(underlyingPrice, b.shortStrike);
...
export function generatePutCreditSpreadCandidates(
  ticker: string,
  underlyingPrice: number,
  ...
  const puts = optionRows.filter((row) => row.optionType === "put");

  if (puts.length === 0) {
    throw new OptionsDataUnavailableError(
      `No live put contracts were available for ${ticker}.`
    );
  }
  ...
  const generated: PutCreditSpreadGeneratedCandidate[] = [];

  for (const [expiration, rows] of grouped.entries()) {
    const dte = getDaysToExpiration(expiration);

    if (
      dte < FIRST_PUT_CREDIT_SPREAD_RULESET.minDte ||
      dte > FIRST_PUT_CREDIT_SPREAD_RULESET.maxDte
    ) {
      continue;
    }
    ...
    for (const shortRow of rows) {
      ...
      const pctBelow = percentBelowSpot(underlyingPrice, shortRow.strike);

      if (
        pctBelow < FIRST_PUT_CREDIT_SPREAD_RULESET.minPercentBelowSpot ||
        pctBelow > FIRST_PUT_CREDIT_SPREAD_RULESET.maxPercentBelowSpot
      ) {
        continue;
      }

      for (const width of FIRST_PUT_CREDIT_SPREAD_RULESET.allowedWidths) {
        ...
        if (netCredit < FIRST_PUT_CREDIT_SPREAD_RULESET.minNetCredit) {
          continue;
        }
        ...
        generated.push({
          ...
          downsideBufferScore: scoreDownsideBuffer(pctBelow),
  ...
  if (generated.length === 0) {
    throw new OptionsDataUnavailableError(
      `No valid live put credit spread candidates were generated for ${ticker} within the current ruleset.`
    );
  }

  const preSorted = preSortCandidates(generated, underlyingPrice).slice(
    0,
    FIRST_PUT_CREDIT_SPREAD_RULESET.maxGeneratedCandidates
  );
